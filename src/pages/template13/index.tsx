...
const Template13: React.FC<{ theme?: ITheme; club: string }> = ({ theme, club }) => {
  ...
  const membershipOptions = [
    ...
    monthPrice && { icon: theme?.monthImg, title: "Monthly Member", price: `${monthPrice} ETH`, type: "month" },
    quarterPrice && { icon: theme?.quarterImg, title: "Quarterly Member", price: `${quarterPrice} ETH`, type: "quarter" },
  ...
  return (
    <div className="flex flex-col items-center w-full min-h-screen overflow-hidden bg-white">
          ...
          {[theme?.clubIntroduction1, theme?.clubIntroduction2].map((txt, i) => (
            <div key={i} className="flex items-start gap-4">
              <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center">
                <img src={theme?.heroImg} className="w-6 h-6 rounded" />
              </div>
              <div className="flex-1">
                <p className="text-sm text-white/90">{txt}</p>
                <button className="mt-2 text-white font-bold uppercase text-sm">View more</button>
                <div className="h-[2px] bg-[#f8cf01]" />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="w-full bg-[#eb2832] py-12">
          <div className="max-w-[1180px] mx-auto px-6">
            <div className="text-center text-white">
              ...
              <h2 className="text-3xl md:text-4xl font-extrabold uppercase">Join the Option</h2>
            </div>
            <div className="mt-8 grid grid-cols-1 md:grid-cols-4 gap-6">
              {membershipOptions.map((opt) => (
                <div key={opt.title} className="flex items-center gap-5 bg-transparent">
                  <img src={opt.icon || "/lifetime.png"} className="w-20 h-20 rounded-full" />
                  <div className="flex-1 flex flex-col gap-3">
                    <div>
                      <p className="text-white text-sm">{opt.title}</p>
                      <p className="text-white text-2xl font-extrabold uppercase">{opt.price}</p>
                    </div>
                    <button className="inline-flex items-center gap-2 bg-[#f8cf01] text-black rounded px-4 py-2 font-bold uppercase shadow" onClick={() => handleJoin(opt.type)}>
                      Join now
                      <img src="/arrow_right_circle_fill.png" className="w-6 h-6" />
                    </button>
                  </div>
                </div>
              ))}
        ...
        <div className="w-full py-16">
          <div className="max-w-[1180px] mx-auto px-6">
            <div className="text-center">
              <p className="text-sm">meta text</p>
              <h2 className="text-3xl md:text-4xl font-extrabold uppercase">Position Verification</h2>
            </div>
            <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
              {verifyData.map((it, idx) => (
                <div key={idx} className="flex flex-col gap-4 border border-black/40 bg-white p-5 rounded">
                  <div className="w-full bg-[#f4f4f4] h-[60px] flex items-center justify-center rounded">
                    <img src={theme?.verifyImgs?.[idx] || theme?.verifyImgs?.[0] || "/aave.png"} className="h-10" />
                  </div>
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="font-bold">{it.chainName} Chain</p>
                      <p className="text-black/80 text-xs">Hold {it.tokenSymbol} ≥ {formatUnits(it.threshold, it.decimals)}</p>
                    </div>
                    <button className="inline-flex items-center gap-2 bg-[#f8cf01] text-black rounded px-4 py-2 font-bold uppercase shadow" onClick={() => handleVerify(it)}>Verify</button>
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* Links & Apps */}
      <div className="w-full py-20 bg-[#fde36a] -skew-y-2">
        <div className="max-w-[1180px] mx-auto px-6 skew-y-2">
          <div className="text-center mb-8">
            <p className="text-sm">meta text</p>
            <h2 className="text-3xl md:text-4xl font-extrabold uppercase">Links & Apps</h2>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-6 gap-6">
            {theme?.socials?.map((app, i) => (
              <div key={i} className="flex flex-col items-center gap-3 bg-white rounded p-6 shadow">
                <div className="w-16 h-16 rounded-2xl flex items-center justify-center" style={{ background: ["#3088ff", "#6aabe9", "#778cd3", "#eb3323", "#04231e", "#01cd88"][i % 6] }}>
                  <img src={app.icon} className="w-8 h-8" />
                </div>
                <p className="font-bold">{app.name}</p>
                <button className="inline-flex items-center gap-2 bg-black text-white rounded px-4 py-2" onClick={() => window.open(app.link, "_blank")}>{app.text}</button>
              </div>
            ))}
          </div>
        </div>
      ...
      {theme?.news && theme.news.length > 0 && (
        <div className="w-full py-16">
          <div className="max-w-[1180px] mx-auto px-6">
            <div className="text-center mb-8">
              <p className="text-sm">meta text</p>
              <h2 className="text-3xl md:text-4xl font-extrabold uppercase">Community News</h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {currentNewsData.map((news, idx) => (
                <div key={idx} className="flex flex-col gap-3 border border-black/20 rounded bg-white p-5">
                  <img src={news.image} className="w-full h-[170px] object-cover rounded" />
                  <p className="font-bold">{news.title}</p>
                  <p className="text-black/70 text-sm">Source: {news.source}</p>
                  <button className="text-[#eb262d] font-bold uppercase text-sm w-fit" onClick={() => window.open(news.link, "_blank")}>View more</button>
